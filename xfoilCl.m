...
%     C = textscan(fid, '%10f%9f%f', 'Delimiter', '', 'WhiteSpace', '','HeaderLines' ,3, 'ReturnOnError', true); 
      C = textscan(fid, '%10f%9f%f', 'Delimiter', '', 'WhiteSpace', '', 'HeaderLines' ,3,'TreatAsEmpty',{'*********\n'});
      fclose(fid);
...
%        foil.cp(:,jj) = C{3}(:,1); THIS IS THE ORIGINAL LINE
                sizea = length(foil.cp(:,jj));
                sizeb = length(C{3}(:,1));
                if sizea~=sizeb
                    foil.cp = zeros(sizeb,Nalpha);
                    foil.cp(:,jj) = C{3}(:,1);
                else
                    foil.cp(:,jj) = C{3}(:,1);
                end
